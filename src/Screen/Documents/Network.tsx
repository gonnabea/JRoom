        ...
        <ContentArea>
          ...
          <ContentMainArea>
            <WikiContentBox
              ...
              대표적인 차이점은, webSocket은 클라이언트가 요청을 보내면 --중계서버를 통하여-- 해당 요청에 대한 응답을 타 모든 클라이언트에게 전달해주는 방식입니다. 그러나 WebRTC는 클라이언트와 클라이언트가 --서버를 통하지 않고-- 서로 연결되어 (Peer to Peer) 빠른 응답속도를 낼 수 있는 것입니다.
              
              유튜브에서 본 WebRTC를 설명하는 내용, 흥미롭게도 영상에서 webSocket과 비교해주며 설명해줍니다. https://www.youtube.com/watch?v=2Z2PDsqgJP8&t=421s
              
              WebRTC에 대한 기타 내용 정리
              
              어떻게 서버를 통하지 않고 클라이언트끼리 연결을 하는가? 최초에 서버가 각각 클라이언트들에게 연결하는 과정을 진행하고 그 후에는 관여하지 않습니다. 해당 프로젝트에서는 최초에 socket.io로 도움을 받아 클라이언트끼리 연결해주는 과정을 거칩니다
              
              중계과정
              
               Indentify (클라이언트 인식, 검증)
              
               Type of Data (전송받은 데이터의 타입을 체크: 비디오인지 오디오인지 등등)
              
               NAT traversal (Peer to Peer 컨넥션: 클라이언트간 연결해주는 기술이라고 하는데 자세히는 모름)
              
               Security (데이터 Encrypt 과정)
              
               Codec (데이터 압축)
              
              특징
              
               주로 UDP를 사용: TCP와 UDP의 차이는 속도와 데이터 전송의 안정성에 주로 있는데, UDP는 안정성보다는 속도에 특화된 프로토콜이다 따라서 스트리밍 서비스나 액션 게임등에 적합합니다 (webSocket은 TCP를 사용)
              
               표준 프로토콜이 없습니다
              
               일부 브라우저에서 지원이 되지 않습니다 (ex) Firefox -> 그러나 adapter.js 라는 것을 이용하면 호환성을 개선할 수 있는 것으로 보입니다.
